...
type Step = 'intro' | 'pay' | 'id' | 'income' | 'review' | 'done'
...
export default function ScreeningFlow({ applicationId, applicantName, applicantEmail, requirements }: Props) {
  ...
  if (step === 'income' && orderId) {
    return <IncomeStep orderId={orderId} onContinue={finishUploads} />
  }
...
// ── Intro card ─────────────────────────────────────────────────────────
function IntroCard({ applicantName, onStart, onSkip, requirements, error }: {
  applicantName: string
  onStart: () => void
  onSkip: () => void
  requirements: ScreeningRequirements
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-gradient-to-br from-brand-500 to-brand-600 text-white rounded-2xl p-7 mb-5">
        ...
        <p className="text-sm text-white/90 mt-2 leading-relaxed">
          ...
          the landlord's review queue with a rentability score — most get a decision in under 24 hours.
        </p>
      ...
      <div className="mt-3 flex flex-col sm:flex-row gap-3">
        <button
          type="button"
          onClick={onStart}
          className="flex-1 inline-flex items-center justify-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-medium px-6 py-3 rounded-lg transition-colors"
        >
        ...
        <button
          type="button"
          onClick={onSkip}
          className="px-5 py-3 rounded-lg text-sm font-medium text-mute hover:text-ink border border-gray-200 hover:border-gray-300 transition-colors"
        >
          Skip for now
        </button>
      </div>
