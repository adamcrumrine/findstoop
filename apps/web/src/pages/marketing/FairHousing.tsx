    ...
    <div className="max-w-2xl mx-auto py-10 px-5">
      ...
      <section className="mt-8 space-y-6 text-sm leading-relaxed text-ink">
        ...
        <Sec title="Equal Housing Opportunity">
          {BRAND.legalName} is committed to the principle of equal housing opportunity. We do not discriminate, and we do not
        ...
        <Sec title="Federal Fair Housing Act">
          <p>
            {BRAND.legalName} is a fair housing advocate. The federal Fair Housing Act (42 U.S.C. §§ 3601–3619) prohibits
          ...
          <p className="mt-2">
            ...
            criminal history (HUD guidance under disparate impact), and others. <strong>{BRAND.legalName} expects landlords using
            the platform to comply with the broadest applicable set of protections.</strong>
          </p>
        </Sec>

        <Sec title={`What this means for landlords using ${BRAND.name}`}>
        ...
        <Sec title={`What this means for renters using ${BRAND.name}`}>
          <p>
            You have the right to be free from housing discrimination. If you believe a landlord, listing, or application
            process on {BRAND.name} discriminated against you on the basis of a protected class:
          </p>
          <ul className="list-disc pl-5 space-y-1.5 mt-2">
            ...
            <li><strong>Report the listing or landlord to {BRAND.name}</strong> at <a href={`mailto:${BRAND.supportEmail}`} className="text-brand-600 hover:underline">{BRAND.supportEmail}</a> with subject "Fair Housing Concern" so we can investigate and, where warranted, suspend the landlord's account.</li>
          </ul>
        </Sec>
        ...
        <Sec title="Reasonable accommodations + modifications">
          ...
          uploads — contact <a href={`mailto:${BRAND.supportEmail}`} className="text-brand-600 hover:underline">{BRAND.supportEmail}</a>{' '}
        ...
        <Sec title="Source-of-income protections">
          ...
          Landlords using {BRAND.name} are responsible for knowing and complying with any source-of-income protections in
          their jurisdiction. {BRAND.name}'s screening tools accept and consider all lawful income sources equally.
        </Sec>
